...
int safetyflag() 
{
  ...
  int stat = GPIO.digitalRead(ESTOP); //read input pin
  if (stat == GPIO.HIGH)
  {
    //return flag value
    return 1;
  ...
  stat = GPIO.digitalRead(DEFEAT_SAFETY); //read input pin
  if (stat == GPIO.HIGH)
  {
    //return flag value
    return 2;
  ...
  stat = GPIO.digitalRead(SAFETY_CIRCUIT); //read input pin
  if (stat == GPIO.LOW)
  {
    //return flag value
    return 3;
...
int ready2fire()
{
  ...
  int stat = GPIO.digitalRead(THRESHOLDS); //read input pin
  ...
  stat = GPIO.digitalRead(SHUTTER); //read input pin
  ...
  stat = GPIO.digitalRead(FIBER); //read input pin
