...
*/
  ...
  // sign displayed when laser is fired is dependent on if threshold is on or off. Pulled from main if loop to refresh image if threshold is switched
  if (states.get(LASER_FIRE_MASK)){
      if (states.get(THRESHOLD_MASK)){
  ...
  // read program, shutter, and threshold masks and overlay textbox and text on signage
